...
> 截至 `2026-06-16` UTC，当前正式入口已进一步收敛到 `full22` 顶层候选壳终审后的 `authoritative snapshot=snapshot:2026-06-15.full.18`、`published=332/444`、`all-candidates=334/447`、`candidate-only=2/3`，且 `real_data_launch` 仍等待基于外部 Neo4j/Redis 的 `source=neo4j` 正向闭环。
