...
本文件定义 `real_data_launch` 的正式闭环验收方式。目标是基于 `snapshot:2026-06-15.full.13` 正式包，在真实 `Neo4j + Redis` 依赖上完成：
...
- package snapshot：`snapshot:2026-06-15.full.13`
- `relations.jsonl`：`252` 行
- `evidence.jsonl`：`331` 行
...
- `--skip-preview-baseline`
  - 跳过 preview/default 的 JSON 与 live 失败语义基线检查
