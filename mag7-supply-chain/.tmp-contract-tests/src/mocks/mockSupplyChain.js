...
const companyListItems = companies.map((company) => ({
    id: company.id,
...
const evidenceById = {
    ...
    "evidence:tesla-battery-passport": {
        id: "evidence:tesla-battery-passport",
        sourceType: "official_doc",
        title: "Tesla Battery Supply Chain Due Diligence Statement",
        publisher: "Tesla",
        url: "https://www.tesla.com/legal/supply-chain",
        publishedAt: "2025-11-01T00:00:00.000Z",
        publishedAtResolution: "day",
        retrievedAt: "2026-06-14T00:00:00.000Z",
        excerpt: "Tesla publishes formal battery material and supplier due diligence disclosures.",
        pageRef: "Battery materials due diligence",
        language: "en",
        hash: "sha256:tesla-battery-passport",
        sourceDomain: "tesla.com",
        citationText: "Tesla publishes formal battery material and supplier due diligence disclosures.",
        reliabilityTier: 1,
        licenseNote: null,
        parserVersion: "mock-1.0.0",
    },
...
const subgraph = {
    ...
    relations: [
        relation({
            ...
            evidenceIds: ["evidence:tesla-10k", "evidence:panasonic-report", "evidence:tesla-battery-passport"],
            primaryEvidenceId: "evidence:tesla-battery-passport",
            evidenceCount: 3,
            ...
            validTo: null,
            evidence: [
                evidenceById["evidence:tesla-10k"],
                evidenceById["evidence:panasonic-report"],
                evidenceById["evidence:tesla-battery-passport"],
            ],
        }),
        relation({
            id: "rel:tesla-catl",
        ...
        relation({
            ...
            status: "approved",
            sourceMethod: "mock_frontend_fixture",
            evidenceDateResolution: "quarter",
            sourceCount: 1,
...
export function getCompaniesResponse(query) {
    const normalized = query?.trim().toLowerCase();
    const items = normalized
        ? companyListItems.filter((company) => company.name.toLowerCase().includes(normalized) ||
...
export function getCompanyResponse(companyId) {
    return {
        item: getCompany(companyId),
        source: "mock",
    };
}
export function getCompanyOverviewResponse(companyId) {
    return overviewByCompanyId[companyId] ?? overviewByCompanyId["company:TSLA"];
}
export function getSubgraphResponse(companyId, depth, includeEvidence) {
    const filteredRelations = subgraph.relations.filter((relation) => relation.depthFromMag7 <= depth &&
    ...
    return {
        ...
        relations: filteredRelations.map((relation) => includeEvidence ? relation : { ...relation, evidence: undefined }),
    };
}
export function getRelationEvidenceResponse(relationId) {
...
function touchesCompanyChain(relation, companyId, depth) {
